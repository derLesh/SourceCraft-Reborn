        ...
        extends Stairs
{
  ...
  public void getType(Point p)
  {
    ...
    else if (this.map.hasOrHadMaterial(x, y, z - 1, new StairsEast().getMaterialUsedFor()))
    {
      setSubBlock(SubBlockPosition.TOP_EAST_SOUTH);
      setSubBlock(SubBlockPosition.TOP_EAST_NORTH);
    }
    else if (!this.map.hasOrHadMaterial(x, y, z + 1, new StairsWest().getMaterialUsedFor()))
    {
      setSubBlock(SubBlockPosition.TOP_EAST_SOUTH);
    }
    if (this.map.hasOrHadMaterial(x - 1, y, z, getMaterialUsedFor()))
