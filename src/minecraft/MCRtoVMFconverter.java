        ...
        extends JFrame
{
  ...
  {
    ConfigWriter writer = new ConfigWriter(this.config);
    writer.writeConfig();
    if (!this.config.placesEmpty())
    {
      PlacesWriter placesWriter = new PlacesWriter(this.config);
  ...
  public void run2()
  {
    System.out.println(this.config.variablesToString());
    SourceGame[] games = this.config.getGames();
    ...
    {
      SourceGame game = this.config.getGame(order);
      if (game == null) {
  ...
  {
    File materiaPath = game.getMatriealPath(this.config, pack);
  ...
  public void moveFolder(File materiaPath)
  {
    ...
    int n = JOptionPane.showOptionDialog(null, "Do you want to update \"" + this.config.getPack() + "\"-textures?\nFiles in \"" + materiaPath + "\\\" may be overwritten.", "There are new " + this.config.getPack() + "-textures.", 1, -1, null, options, options[1]);
    if (n == 0)
    {
      File srcFolder = new File("textures\\" + this.config.getPack());
  ...
  {
    Place[] allPlaces = this.config.getPlaces();
    ...
    return getMinecraftMap(place, game, this.config.getDefaultConvertOption(game), new TexturePack());
  }
  ...
  private class EndingFilenameFilter
          implements FilenameFilter
  {
    private String ending;

    public EndingFilenameFilter(String ending)
    {
      this.ending = ending;
    }
    ...
    public boolean accept(File dir, String name)
    {
      ...
      if (parts.length > 0) {
        return parts[(parts.length - 1)].equals(this.ending);
      }
      return false;
    }
  }
  ...
  {
    File result = this.config.getWorldFile(place);
    if (!result.exists())
    {
