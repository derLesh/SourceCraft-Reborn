  ...
  extends Writer
{
  ...
  public void writeConfig() {
    writeFinalArgument("version", config.getVersion(), true);
  ...
  private void writePaths() {
    writeFinalArgument("minecraft", config.getMinecraftPathString(), true);
    writeFinalArgument("steam", config.getSteamPathString(), true);
    writeFinalArgument("steamUserName", config.getSteamUserName(), false);
  }
  
  private void writeWindow()
  {
    writeFinalArgument("xPos", config.getWindowPosX(), true);
    writeFinalArgument("yPos", config.getWindowPosY(), false);
  }
  
  private void writeVariables(boolean successor) {
    initiateSection("variables");
    
    writeFinalArgument("place", config.getPlace(), true);
    writeFinalArgument("game", config.getGame(), true);
    writeFinalArgument("convertOption", config.getConvertOption(), true);
    writeFinalArgument("texturePack", config.getPack(), false);
  ...
  private void writeGames(boolean successor)
  {
    ...
    SourceGame[] game = config.getGames();
  ...
  private void writeGame(SourceGame game, boolean successor) {
    ...
    writeFinalArgument("gamePath", game.getGamePathString(config), true);
  ...
  private void wrtieConvertOptions(boolean successor) {
    ...
    ConvertOption[] options = config.getConvertOptions();
